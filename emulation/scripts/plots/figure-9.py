...
with open(f'{caravan_home}/emulation/scripts/results/figure9-experiment.jsonl', 'r') as result_file:
    for record in result_file:
        data = json.loads(record)
        if data["retrain_frequency"] > 1:
            my_accuracy_gain.append(data["my_improvement"])
            my_gpu_cycle.append(data["gpu_time"])
        else:
            cts_accuracy_gain.append(data["my_improvement"])
            cts_gpu_cycle.append(data["gpu_time"])
...
# Plotting the lines
plt.figure()
plt.plot(my_gpu_cycle, my_accuracy_gain, marker='o', linewidth=3, markersize=10, label='Selective retraining', color=colors[0])
plt.plot(cts_gpu_cycle, cts_accuracy_gain, marker='o', linewidth=3, markersize=10, label='Continuous retraining', color=colors[1])
...
plt.savefig(f"{caravan_home}/emulation/scripts/figures/figure-9.png")