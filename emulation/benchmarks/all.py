...
def icarl_cap(features, labels, labeling_window_size, upper_limit_proportion=0.2):
    num_samples = int(labeling_window_size * upper_limit_proportion)
    if num_samples > features.shape[0]:
        return features, labels
    
    sampled_indices = torch.multinomial(labels.float(), num_samples, replacement=False)
    sampled_features = features[sampled_indices]
    sampled_labels = labels[sampled_indices]
    return sampled_features, sampled_labels


def label_flows_with_rule_cache(features, rule_cache_function_def):

    namespace = {}
    exec(rule_cache_function_def, namespace)
    label_flow_with_rule_cache = namespace['label_flow_with_rule_cache']

    labels = []
    for flow_index in range(features.shape[0]):
        flow = features[flow_index, :]
        label = label_flow_with_rule_cache(flow)
        labels.append(label)
...
def retraining_trigger(window_index, retrain_flags, 
    ...
    if retrain_flags[-1] == False:
        ...
        # if my_model_f1s_proxy[-2] - my_model_f1s_proxy[-1] > 0.1:
            return True
        else:
            return False
...
def param_to_csv(parameters_tensor, csv_path):
    parameters_np = parameters_tensor.cpu().numpy()
    parameters_df = pd.DataFrame(parameters_np)
    parameters_df.to_csv(csv_path, index=False, header=False)
...
def reset_model(model, optimizer, lr, static_model=None):
    if static_model:
        model = copy.deepcopy(static_model)
        optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9)
    else:
        print("model not reset")
...
def continuous_retrain(args):
    ...
    static_dnn.eval()
    cts_retraining_dnn = copy.deepcopy(my_dnn)
    cts_retraining_dnn.eval()
    ...
    retrain_flags = []
    use_retraining_trigger = True if args.retraining_trigger == 1 else False
    ...
    for k, (features, gt_label, raw_features, generated_label) in enumerate(streaming_dataloader):
        ...
        # ====================================================================
            ...
            # model revert: do not use it for iot traffic classification
            # if args.application_type != "iot_traffic_classification":
            #     if my_window_f1 < 0.2:
            ...
            if not use_retraining_trigger or \
                retraining_trigger(window_index, retrain_flags, my_model_f1s_proxy, static_model_f1s_proxy):
                ...
                if args.labeler_type != "device_list":
                    ...
                    training_features, training_labels = icarl_cap(valid_features, valid_labels, args.eval_frequency)
            ...
            if args.labeler_type != "device_list":
                ...
                oracle_training_features, oracle_training_labels = icarl_cap(online_learning_features, online_learning_labels, args.eval_frequency)
    ...
    result_json = {
        ...
        "retraining_trigger": args.retraining_trigger,
        "my_improvement": improvement_us_avg,
        "my_gpu_total_time": my_gpu_total_time,
        "gt_improvement": improvement_gt_avg,
        "gt_gpu_total_time": gt_gpu_total_time,
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--retraining_trigger",
        type=int,
        help="Whether we should use retraining trigger or not",
        default=1,
    )
    parser.add_argument(
        "-e",
    ...
    # initialize logging
    if not os.path.exists(os.path.dirname(args.logdir)):
        os.makedirs(os.path.dirname(args.logdir))
    initialize_logging(args.logdir)
